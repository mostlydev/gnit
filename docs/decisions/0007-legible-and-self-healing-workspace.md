...
`gnit status` only listed member ids and paths, and there was no top-level
`gnit log`. The transparent upkeep hook was wired but a no-op.
...
- **Rich `gnit status`** reports the workspace root and each member: staged /
...
- **`gnit log`** renders one interleaved, newest-first timeline of Changes
  (reconstructed from `Gnit-Change-Id` trailers across members) and Pins, each
  with a UTC date. This is the operator's "retrievable shared graph" as a single
  command. `gnit change log` remains the change-only view.
...
- **Real upkeep.** The transparent upkeep hook now repairs the root repo's local
  ...
  / `GNIT_NO_UPKEEP` still disables it.
