# Gnit: Change Groups, Pins, Checkout, Governance

## Status

Converged v1/v2 design after adversarial review with Claude and Codex.

The product name is **Gnit**: a small Git-native layer for knitting
...
Gnit v1 addresses that directly.

It does **not** create a new VCS. Each member remains a normal Git repository.
Gnit adds shared workspace metadata, commit grouping, reproducible pins, safe
...
|---|---|---|---|
| Change | Which commits are one logical change? | Commit trailers in member repos | `gnit commit`, `gnit change show` |
| Pin | Which exact commits should this workspace materialize? | Committed pin artifact in the root/control repo | `gnit pin`, `gnit checkout` |
...
`{node -> commit}` *selection* plus provenance. Gnit records what belongs together
...
- If no natural root exists, `gnit init` can create a tiny workspace-control repo.
...
- `gnit init` runs in a directory. If that directory **is itself a Git repo**, it
  becomes the root member and hosts `.gnit/`.
- If it **contains** repos but is not one (e.g. sibling repos `app/` and
  `service/` under a plain `workspace/` dir), there is **no natural root**: `gnit
...
The roster changes rarely: `gnit adopt`, `gnit ignore`, path moves, and optional
...
Gnit only wins if the common publish path is shorter and less stateful than
...
Gnit's regular publish path must be:

```text
gnit add -A                                   # or explicit paths
gnit land -m "Publish webhook retry update"   # name is OPTIONAL
gnit push
```

`gnit land` is the important ergonomic command. It plans and performs the whole
publish transaction:

1. commit staged changes in each touched member repo with one `Gnit-Change-Id`
2. create a Pin for the resulting three-repo state
3. commit the Pin in the root/control repo
4. leave `gnit push` to publish member commits first and the Pin last

The Pin **name is optional**: `gnit land -m "..."` auto-generates a pin id;
`gnit land <name> -m "..."` names it for a release. This matters: submodules make
...
- **`gnit land`** - "publish this workspace state" for a dependency-coupled
...
- **`gnit commit`** (no pin) - WIP, incremental work, or loosely-coupled repos
  ...
  one. `gnit commit` + `gnit pin` remain available as the decomposed form.

If a user regularly has to run child commit, root metadata commit, and ordered
push by hand, Gnit has failed its purpose.
...
created control repo at product/  (.gnit/ created, origin set)
...
$ gnit add app/src/webhooks.ts sdk/src/retry.ts docs/retry.md
staged: app(1), sdk(1), docs(1)

$ gnit land -m "Publish webhook retry update"
plan:
  Change GCH-20260603-k9d2
    app   commit staged changes
    sdk   commit staged changes
    docs  commit staged changes
  Pin PIN-20260603-p4  (unnamed; add `gnit land <name>` to label a release)
    app   <new app commit>
    sdk   <new sdk commit>
    docs  <new docs commit>
proceed? yes

created Change GCH-20260603-k9d2
...
is reduced to staging, landing, and pushing. Gnit carries the pointer/pin update,
metadata commit, and push-order bookkeeping.

### Being honest: where submodules are comparable or ahead

Gnit is not a free win everywhere, and pretending otherwise would be a sales pitch.
...
- **Reconstruction must stay one command.** `git clone --recursive` hydrates a
  whole submodule tree in one step. So `gnit clone <url>` **auto-hydrates** members
...
- **For truly unrelated repos** that just share a directory, both Gnit and a
...
The verdict: for a *related* multi-repo workspace, Gnit's basic publish is fewer
...
are part of the design surface: they show where Gnit must avoid surprising Git
users.
...
Walkthrough (real session; note that **Gnit commits its own metadata**; the user
never hand-stages `.gnit/`):

```console
$ cd app
$ gnit init
initialized Gnit workspace  (root: app, created .gnit/)
discovered 1 nested repo not yet adopted:
  vendor/sdk   -> run `gnit adopt vendor/sdk` to manage it

$ gnit adopt vendor/sdk --id sdk
...
- `gnit adopt` records `sdk` in the shared roster, writes the local excludes so
...
- Roster and pin commits are plain metadata commits in the root/control repo;
  they do not carry a `Gnit-Change-Id` and are never folded into a code Change.
- `gnit doctor` can reapply the excludes on another machine from the shared roster.
- `gnit pin baseline` creates the first reproducible snapshot and commits it.

The user should not have to stage a child gitlink, edit a manifest, or hand-run
`git add .gnit/...`. If a step in these walkthroughs would make the user reach for
raw `git` to manage Gnit metadata, that is a faceplant and the command owns it
instead.
...
$ cd workspace
$ gnit init --control --remote git@github.com:example/product-workspace.git
created control repo at workspace/  (.gnit/ created, origin set)
...
- `gnit init --control [--remote <url>]` creates a tiny Git repo at `workspace/`
  to hold `.gnit/`, and records the remote if given (no raw `git remote add`).
- `gnit adopt` accepts multiple paths and commits one roster update for the batch.
...
- **`gnit clone` auto-hydrates by default**: it clones the root/control repo,
  reads `.gnit/roster.yaml`, clones any missing members to their roster paths,
...
- `--pin <p>` additionally materializes members to that pin's commits with the
  same safe rules as `gnit checkout` (refuse unsafe resets or dirty worktrees).
- Without `--pin`, members land on their default branches. There is no separate
  `hydrate` verb: re-running `gnit checkout <pin>` clones any member added later,
  and `gnit doctor` repairs a partial workspace.
- If a member remote is missing or a pinned commit is unreachable, Gnit reports the
...
There is **no `gnit new` verb**. Make the repo with the Git verb you already know,
then adopt it. This keeps the surface git-native and avoids a `gnit new` /
`adopt --init` mode flag:

```console
$ git init plugins/payments          # or: git clone <url> plugins/payments
$ gnit adopt plugins/payments --id payments
...
- `gnit adopt` works on any repo already on disk, however it was created (`git
...
- `gnit status` shows the new member as present but unpinned until a Pin records
  its commit.
...
- `git clone` puts the repo on disk with its remote already configured; `gnit
...
```text
gnit import-submodule vendor/sdk --id sdk
gnit status
gnit pin baseline
```
...
- Gnit detects the existing gitlink and `.gitmodules` entry.
...
Walkthrough (run from anywhere inside the workspace; `gnit` finds the root by
walking up to `.gnit/`, just like `git` finds `.git`):

```console
$ # edited files in app and sdk
$ gnit add sdk/src/retry.ts app/src/webhooks.ts app/tests/webhooks.ts
staged: sdk(1), app(2)
...
$ gnit land -m "Add webhook retry support"
plan:
  Change GCH-20260603-abc123
    sdk  commit staged changes
    app  commit staged changes
...
created Change GCH-20260603-abc123
...
- `gnit land -m "message"` is the canonical publish path: one planned operation
  for member commits plus the reproducible Pin. `gnit land <name> -m "message"`
  is the named form for releases or human handles.
- `gnit commit` alone does not automatically create a Pin. It reports pin drift
  so the user knows the workspace has unpinned changes.
- `gnit pin` is still a deliberate reproducibility act. The decomposed form is:
  `gnit commit -m "message"` followed by `gnit pin <name>`.
...
```text
gnit log
gnit pin release-2026-06-03
gnit diff previous-release release-2026-06-03
gnit review release-2026-06-03
gnit push
```
...
- `gnit diff <pinA> <pinB>` shows what changed between two reproducible states
  even when no single Change produced the new state.
- `gnit review <pin>` produces the review artifact for the release.
...
```console
$ gnit worktree add ../hotfix --pin production
materialized workspace at ../hotfix from pin 'production'
  app  detached @ def456  (worktree)
  sdk  detached @ abc123  (worktree)
  root control worktree on branch gnit/wt/hotfix

$ cd ../hotfix
$ # edit only app
$ gnit add app/src/patch.ts
staged: app(1)
$ gnit commit -m "Patch production webhook timeout"
created Change GCH-20260603-hf01
  app  fed210  Patch production webhook timeout
$ gnit pin production-hotfix
...
- `gnit worktree add` creates a separate materialized workspace rooted at
...
- Worktree branch collisions are handled, not hit: a member is materialized
  **detached at the pinned commit** (or on a fresh `gnit/wt/<name>` branch), so it
...
- The root/control repo is also a worktree, on its own `gnit/wt/<name>` branch, so
  pins/changes created here are ordinary commits that push independently.
- It materializes the requested Pin with the same safe rules as `gnit checkout`.
...
```text
gnit worktree add ../attempt-a --pin baseline
cd ../attempt-a
# harness creates per-touched-repo branches lazily
gnit commit -m "Attempt retry implementation"
gnit pin attempt-a-ready
gnit review attempt-a-ready
```

Required behavior:

- Gnit does not create same-named branches in every repo.
- The harness may create branches only in repos it edits, for example
  `gnit/GCH-20260603-abc123/app`.
...
resolve app, then run: gnit push --resume
...
Gnit pushed members first, and when `app` failed it **held the pin back** instead
...
```console
$ gnit checkout release-2026-05
  app   ok @ def456
  sdk   ! pinned commit abc123 is unreachable (history rewritten?)
checkout stopped: pin 'release-2026-05' is dangling in sdk. run `gnit doctor`.
...
this pin cannot be materialized as-is. options:
  - re-pin from the surviving SHA:   gnit pin --change <id> release-2026-05b
  - recover abc123 from a reflog/backup remote and re-run gnit checkout
  - record the pin as superseded (keeps history honest; not reproducible)
```

Recovery never invents evidence. `gnit doctor` explains the break and offers
explicit actions; it does not silently repoint a pin to a different commit.

### UX checks these examples force

- There must be a one-command fresh-machine path: `gnit clone --pin` auto-hydrates
  members and materializes the pin, matching `git clone --recursive`; never three
  separate steps.
- New members use plain `git init`/`git clone` then `gnit adopt` - no `gnit new`
  verb and no `adopt --init`/`--clone` mode flags (keeps the surface git-native
  without confusing arguments).
- There must be a submodule migration path: `gnit import-submodule` (its own verb,
  not a flag on adopt, because it is migration surgery, not adoption).
- There must be an isolated workspace path: `gnit worktree add`.
- There must be an explicit staging helper: `gnit add`.
- `gnit commit` must remain staged-only and must not auto-pin by default.
- Pins must push after member commits.
- `gnit status` must always show dirty repos, missing repos, pin drift, unpushed
  participants, ambiguous Change projections, and discovered-but-unadopted repos.
- **Gnit owns its metadata commits.** `adopt`, `import-submodule`, `pin`, and
  label moves commit their own `.gnit/` changes (with `--no-commit` to batch).
  If a walkthrough makes the user hand-run `git add .gnit/...`, that is a faceplant.
- Every command resolves the workspace from the current directory upward, so the
  user is never forced to `cd` back to the root.
- `gnit worktree add` must dodge Git's "branch already checked out" error by
  materializing members detached or on a fresh `gnit/wt/<name>` branch.

---

## Staging And Status UX

Staging must be ergonomic without making `gnit commit` dangerous.

`gnit add` is an explicit path-based fan-out wrapper:

```text
gnit add app/src/webhooks.ts sdk/src/retry.ts
gnit add --repo sdk src/retry.ts tests/retry.ts
gnit add -A                # explicit: stage all tracked-modified files, every member
gnit add .                 # explicit: stage everything under the current directory
```

Rules:

- `gnit add` maps workspace-relative paths to the member repo that owns them.
- `gnit add --repo <id>` interprets paths relative to that member.
- `gnit add -A` / `gnit add .` are the **explicit** bulk-stage forms for the common
...
`gnit commit` remains staged-only and **never implicitly stages**: there is no
transitive `gnit commit -a` that auto-stages across the workspace. Bulk staging is
always an explicit `gnit add -A`/`.` first. `gnit add` improves daily ergonomics
without turning commit into a surprise workspace-wide `git commit -a`.

`gnit status` must be grouped for scanning. A large workspace cannot be a flat
...
Missing or not checked out
  worker missing locally; run `gnit checkout <pin>` to clone + materialize it

Discovered (not adopted)
  tools/scratch   run `gnit adopt tools/scratch` or `gnit ignore tools/scratch`
...
Gnit refuses graph cycles. If A nests B and B bind-mounts back to A, traversal
...
```text
Gnit-Change-Id: GCH-20260603-abc123
```

This document says "Change-Id" generically, but the implementation should use a
Gnit-specific trailer to avoid colliding with Gerrit-style `Change-Id`.

Optional v1 trailers:

```text
Gnit-Change-Goal: Add retry handling for webhook delivery.
Gnit-Actor: codex
Gnit-Session: harness:...
```
...
- stamps each commit with the same `Gnit-Change-Id`
- never stages automatically across repos
- never applies `git commit -a` transitively
- reports pin drift after the commit

`gnit commit --change <id> -m "message"` appends new commits to an existing
...
`gnit change show <id>` must report ambiguity. It must not silently choose by
...
gnit change diff <id>
gnit change log [<id>]
gnit change revert <id>
```

`gnit change revert <id>` is non-destructive. It runs per-repo `git revert` for
resolved participant tips and creates a new Change with:

```text
Gnit-Reverts-Change-Id: <id>
```
...
Pins live in the root/control repo under a Gnit metadata directory, for example:
...
`gnit commit --pin [<name>] -m "message"` or `gnit land [<name>] -m "message"`:
...
`gnit commit` would reintroduce per-change control-repo churn and proliferate
meaningless pins; a Pin is a deliberate act (`gnit pin` / `gnit land`). A policy
(e.g. `gnit.autopin = release-branches | agents`) can opt specific contexts into
...
moving a name requires an explicit update. This keeps `gnit checkout PIN-...`
...
auditable. `gnit checkout <id>` is reproducible; `gnit checkout <label>` resolves a mutable
...
`gnit checkout <pin>` materializes a Pin. It reuses Git's `checkout` mental model
(make my working state match this), extended across repos. (`gnit sync` is kept
...
- **clone any missing roster members** from their recorded remotes (this is why
  there is no separate `hydrate` verb: `gnit clone` auto-hydrates, and `checkout`
...
but it cannot materialize child repos. Gnit owns cross-repo materialization. That
...
`gnit log` renders a single **interleaved workspace timeline** reconstructed by
scanning member repos (per the roster) for `Gnit-Change-Id` trailers and reading
...
database to lose. `gnit change log` and `gnit pin list` remain the per-axis views;
`gnit log` is the unified one. Where a member is missing locally, `gnit log` says so
...
Branches remain per repository. Gnit does not invent a workspace branch namespace
in v1.

Rules:

- `gnit commit` creates commits only in repos with staged changes.
- It does not create empty same-named branches in every member.
- A Pin records exact commits and optional branch hints.
- `gnit checkout` uses branch hints only for safe materialization.
...
`gnit push` must make non-atomicity visible and recoverable:
...
`gnit status --pin <pin>` and `gnit change status <id>` show whether each
participant is:
...
`gnit diff <pinA> <pinB>` is the cross-repo "what changed between two reproducible
...
`gnit review` produces a review artifact with per-repo diffs, selected commits,
...
- records those required local excludes in the shared roster so `gnit doctor`
  can reapply them on another machine
- refuses if the child path is already tracked in an ancestor

If the child is already tracked, Gnit explains the case:

- tracked normal files
- existing submodule/gitlink
- existing Gnit pin relationship
...
A natural worry: if Gnit links repos together, won't a stray raw `git` command
desync the workspace? Should Gnit inject Git hooks to prevent that?

The answer is that **this is mostly covered by the protocol design, not by
hooks.** Gnit is reconstruct-not-enforce: Git is the source of truth, and Gnit
state is a projection rebuilt from trailers + the roster + pin artifacts. So raw
Git is never catastrophic, only (at worst) untidy until the next `gnit` command
reconciles it:

| Raw action | Effect | How Gnit copes |
|---|---|---|
| `git commit` with no `Gnit-Change-Id` | commit exists, ungrouped | `gnit status`/`gnit log` show it as ungrouped; `gnit doctor` can stamp a trailer with explicit user action |
| `git checkout` / `reset` in a member | working tree drifts from the pin | `gnit status` reports drift; `gnit checkout` re-materializes |
| `git push` of a member | just pushes | fine; if a control pin commit is later pushed before a member, `gnit doctor` reports a dangling pin |
| `git rebase` that drops a trailer | Change link lost | already covered; `gnit doctor`/cache repair, no magic claim |
...
- **Default: no hooks.** Rely on reconstruction plus `gnit status`/`gnit doctor` to
  surface and repair drift.
- **Opt-in convenience - `gnit hooks install`:** a `prepare-commit-msg` that stamps
  `Gnit-Change-Id` even on a raw `git commit` (so grouping survives), plus a
  `pre-push` warning about pin ordering. It must **chain** existing hooks, never
  clobber them.
- **Opt-in strict - `gnit hooks install --strict` / `gnit.strict`:** refuse raw
...
```text
# setup / construction  (new members: use plain `git init`/`git clone`, then `gnit adopt`)
gnit init
gnit init --control [--remote <workspace-url>]
gnit init --local
gnit clone <workspace-url> [path] [--pin <pin>]   # clones + auto-hydrates members; --pin also materializes
gnit adopt <path>... [--id <id>] [--no-commit]    # adopt repo(s) already on disk
gnit ignore <path>
gnit import-submodule <path> [--id <id>]          # migrate a submodule to a member
gnit hooks install [--strict]                     # opt-in; default is no hooks
gnit doctor
...
gnit add <path>...
gnit add -A | .                                   # explicit bulk stage across members
gnit add --repo <id> <path>...
gnit commit -m <msg>                               # everyday: group staged changes, no pin
gnit commit --change <id> -m <msg>                 # append to an existing Change
gnit land [<name>] -m <msg>                        # CANONICAL publish: commit + pin (name optional)
gnit commit --pin [<name>] -m <msg>                # equivalent of land; scriptable form, not taught first

gnit show <change|pin>
gnit diff <change|pin>
gnit diff <pinA> <pinB>
...
gnit pin show <pin>
gnit pin diff <pin>
gnit pin diff <pinA> <pinB>
gnit pin list

gnit checkout <pin>
gnit checkout <pin> --exact

gnit worktree add <path> --pin <pin>
...
Recursion is a workspace default for Gnit commands. `--local` or `--repo-only`
is the escape hatch.
...
|---|---|
| Nested repo appears as untracked in parent | `gnit adopt` writes local excludes; `gnit doctor` reapplies them from roster. |
...
| Fresh machine lacks member repos | `gnit clone` auto-hydrates members from roster remotes; re-running `gnit checkout <pin>` clones any member added later; `gnit doctor` repairs a partial workspace and reports unreachable pins. |
| Existing submodule needs migration | `gnit import-submodule` plans removal of gitlink metadata and adoption as a normal member; refuses dirty or stranded submodule states. |
...
| Pin references unpushed member commits | `gnit push` pushes members first and pin commit last. |
| Pin commit pushed but member push failed | This should be prevented by ordering; if remote state contradicts, `gnit doctor` reports a broken pin. |
| Pin references an unreachable member commit | `gnit checkout` stops; `gnit doctor` reports a dangling pin and offers explicit recovery or supersede actions without repointing silently. |
| Plain Git commit without Gnit trailer | It remains normal Git; `gnit status` shows ungrouped commits. `gnit doctor` may attach/recover only with explicit user action. |
...
| Plain Git checkout of root changes pin files | Child repos do not move until `gnit checkout`; status reports drift. |
...
- Implement `gnit init`, `gnit clone` (clones + auto-hydrates members), `gnit adopt`,
  `gnit ignore`, and `gnit import-submodule`. New members are plain `git init`/`git
  clone` followed by `gnit adopt` (no `gnit new` verb).
- Store shared roster in `.gnit/roster.yaml`.
- Make metadata commands commit their own `.gnit/` changes by default, with
...
- Implement `gnit status`.

### Phase 2: Change Grouping

- Implement Change-Id generation and trailers.
- Implement `gnit add` as explicit cross-repo staging.
- Implement `gnit commit` over staged member changes.
- Implement Change projection, ambiguity detection, and views.
- Implement `gnit change revert`.

### Phase 3: Pins

- Implement pin files under `.gnit/pins/`.
- Implement `gnit pin`, `gnit pin --change`, and immutable pin ids (auto-generated;
  name optional).
- Implement `gnit land [<name>]` and `gnit commit --pin` transaction planning.
- Report pin drift in status.

### Phase 4: Checkout

- Implement safe `gnit checkout`.
- Implement `gnit checkout --exact` with confirmation/policy.
- Implement `gnit worktree add <path> --pin <pin>` for isolated materialized
  workspaces, including detached or fresh `gnit/wt/<name>` branch handling to
...
- Implement `gnit show`, `gnit diff`, and `gnit review` for Change and Pin.
- Add `gnit doctor` recovery for trailers, pins, dangling pins, excludes, and
  remote drift.
- Implement opt-in `gnit hooks install [--strict]` (trailer-stamp + pin-order
...
- A v2 Attempt references one or more `Gnit-Change-Id`s.
...
- No hosted web review integration beyond CLI / `gnit review` output.
...
1. **Product name:** `gnit`.
2. **Trailer spelling:** `Gnit-Change-Id` (avoids colliding with Gerrit's
   `Change-Id`); "Change-Id" is shorthand only in prose.
3. **No-natural-root default:** `gnit init --control` creates a tiny workspace
...
4. **Pin labels:** ids are immutable; labels are movable pointers whose moves are
   recorded as commits; `gnit checkout <label>` warns it is resolving a mutable
   pointer.
5. **Metadata commits:** `gnit` commits its own `.gnit/` changes by default
   (`--no-commit` to batch); users never hand-stage metadata.
6. **Hooks:** none installed by default. `gnit hooks install [--strict]` is opt-in;
...
| publish a recorded workspace state | `gnit land` | names a distinct compound op (commit + pin); kept as a verb precisely because hiding it in a forgettable `commit --pin` flag would silently downgrade the operation's meaning |
| register an existing repo as a member | `gnit adopt` | `add` is taken by staging; new members are plain `git init`/`git clone` + `adopt`, with no `gnit new` verb and no `adopt --init`/`--clone` mode flags |
| migrate a submodule | `gnit import-submodule` | migration surgery, not adoption; a self-documenting verb beats a scary flag on `adopt` |
| reproducible cross-repo snapshot | `gnit pin` | an artifact with a commit vector + provenance, not a mere pointer (so not `tag`) |
| combined cross-repo review artifact | `gnit review` | renamed from `bundle` to avoid colliding with `git bundle` |
| diagnose / repair | `gnit doctor` | idiomatic CLI diagnostic |

`gnit sync` is retained only as an alias for `gnit checkout`. There is no `gnit
hydrate` (clone auto-hydrates; checkout clones-missing) and no `gnit new`.
